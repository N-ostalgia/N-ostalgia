...
### BACKEND
```
🐍 Python    ⚡ FastAPI    🌶️ Flask    ☕ Java
🍃 Spring    🔷 C/C++      🟢 Node.js  ⚛️ Electron
🐘 PHP
```
...
### 🏢 XAI TECHNOLOGIES · RABAT (REMOTE)
**AUG–OCT 2025 · Frontend Developer Intern**

```
▸ Built X-Radar, a full React.js network supervision desktop app
▸ Dashboard, analytics, admin view, user settings from scratch
▸ Packaged via Electron.js — cross-platform + SQLite local persistence
▸ Agile / Jira sprint workflow in a fully remote team
```

**Stack:** `React` `Electron` `SQLite` `Chart.js` `Jira`

---

<!-- ══════════════ PROJECTS ══════════════ -->
## 🚀 FEATURED_MISSIONS.load()

<!-- PROJECT 01 -->
<details>
<summary><b>🔐 01. NETWORK INTRUSION DETECTION SYSTEM</b> · AI · CYBERSECURITY</summary>

End-to-end security pipeline. Live traffic → XGBoost → React dashboard via WebSockets. Detects 12+ attack types including DDoS, Brute Force, Botnet, XSS.
...
JavaFX app for tour planning using real OpenStreetMap data. Routes optimised via Greedy, A*, or Genetic algorithms. Sync'd 2D map + 3D robot animation.
...
C++/Qt6 productivity app. OpenCV tracks eye/face presence in real time, computes focus score 0–100%. Gemini API for context-aware tips from session data.
...
Content-based image retrieval. ResNet50 extracts 2048-dim feature vectors, ranked by cosine similarity. Flask API + React upload interface.
...
Web platform automating assignment of teaching units across departments. Supports five distinct roles with permission scopes and dashboards.
...
Command-line banking app in pure C with SQLite persistence. Dual-role system with 14-option client menu. Transaction history via Gnuplot.
...
│ NYU Tandon School of Engineering · 2025             🔍     │
├─────────────────────────────────────────────────────────────┤
│ 🏅 ACHIEVEMENT UNLOCKED                                    │
│ OSINT FUNDAMENTALS                                         │
│ Independent · 2026                                   👁️     │
